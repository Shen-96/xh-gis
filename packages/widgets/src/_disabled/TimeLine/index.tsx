...
import IconDecelerate from "./assets/decelerate.svg";
import IconIncelerate from "./assets/incelerate.svg";
import { FC, RefObject, useEffect, useRef, useState } from "react";
import { CoreType, MathUtils } from "@/lib-client/xh-gis";
      ...
      if (clock) {
        ...
        const container = document
          ...
          timeLine = (timelineRef.current =
            container && new Timeline(container, clock));
    ...
    <div className="xh-gis-viewer-time-line">
      <div className="xh-gis-viewer-time-line-tools">
        <div className="top-bar">
          <IconSvg className="button" title="重新开始">
            <IconStop onClick={handleResetClick} />
          </IconSvg>
          <IconSvg className="button" title="减速">
            <IconDecelerate onClick={handleBackwardClick} />
          </IconSvg>
          <IconSvg className="button" title={!pause ? "暂停" : "开始"}>
            {pause ? (
              <IconBegin onClick={handlePlayClick} />
            ) : (
              <IconPause onClick={handlePlayClick} />
            )}
          </IconSvg>
          <IconSvg className="button" title="加速">
            <IconIncelerate onClick={handleForwardClick} />
          </IconSvg>
        </div>
      ...
      {/* <div className="xh-gis-viewer-time-line-slider">
        <Slider
          min={min}
          max={max}
          value={curValue}
          // onAfterChange={this.afterTimelineChange}
          onChange={onTimelineChange}
          tooltip={{ formatter: () => spendTime }}
        />
      </div> */}
      {/* <div className={"xh-gis-viewer-time-line-tools"}>
        <IconSvg className="button" title={shouldAnimate ? "暂停" : "开始"}>
          {!shouldAnimate ? (
            <IconBegin onClick={handlePlayClick} />
          ) : (
            <IconPause onClick={handlePlayClick} />
      ...
      {/* <div className={"cesium-custom-time-widget-ward"}>
            <IconDe
              className="time-widget-btn"
              onClick={this.handleBackwardClick}
            />
            <IconIn className="time-widget-btn" onClick={this.handleForwardClick} />
          </div> */}
      {/* <div className={"xh-gis-viewer-time-line-tools-info"}>
          {spendTime}/{entireTime}
        </div> */}
      {/* <div className="xh-gis-viewer-time-line-tools-multiplier">
          <div className={"widget-dropdown"}>
            <span>播放速度：</span>
            <Dropdown
              placement="top"
              menu={{ items: multiplierOptions, onClick: onMultiplierClick }}
            >
              <span>X{Math.round(multiplier)}</span>
            </Dropdown>
          </div>
        </div> */}
      {/* </div> */}
